...
const ProfilePicselector = ({ onSelect }: ProfileProps) => {
  ...
  return (
    <div className="profilePicContainer">
      {/*Preview*/}
      <img src={preview} alt="Profile Preview" className="imagePreview" />
      {/*Default choices*/}
      {showGrid ? (
        <div className="defaultAvatarsContainer">
          {defaultAvatars.map((src) => (
            ...
            >
              <img src={src} className="defaultAvatar" alt="Default Avatar" />
            </button>
          ))}
        </div>
      ) : (
    ...
    try {
      ...
      if (selectedImage instanceof File) {
        ...
        const res = await fetch(
          "https://api.cloudinary.com/v1_1/dig44nwql/image/upload",
          {
            method: "POST",
            body: data,
          }
...
const BucketForm = ({ index, onSave }: BucketFormProps) => {
  ...
  const getBucketName = (index: number) =>
    index % 2 === 0 ? "Mission: Not-So-Impossible " : "Plot Twist Pending… ";
...
const BucketManager = () => {
  const { bucketForms, addBucketForm, updateBucketForm } = useAppStore();
  ...
  const handleSave = (index: number, value: BucketFormData) => {
    const isFomValid =
      value.title.trim() && value.description.trim() && value.date.trim();
    if (!isFomValid) {
      alert("Please fill in all fields.");
      return;
    }
  ...
  return (
    <div>
      {bucketForms.map((_, index) => (
        <BucketForm key={index} index={index} onSave={handleSave} />
      ))}
      <AnimatePresence>
                  ...
                  >
                    {editId === item.id ? (
                      ...
                      <>
                        ...
                        <h3>
                          {" "}
                          <span>Your Adventure:</span> {item.title}
                        </h3>
                        <p>
                          {" "}
                        ...
                        <p>
                          {" "}
                          <span>Wish Deadline:</span> {item.date}
                        </p>
                        <input
                          type="checkbox"
                          onChange={() => {
